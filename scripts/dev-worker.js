...
app.post("/notifications", (req, res) => {
  if (!req.body || Object.keys(req.body).length === 0) {
    console.error(
      "Error: Received empty request body. Ensure Content-Type is application/json and data is correctly formatted.",
    );
    return res
      .status(400)
  ...
  res
    .status(200)
    .json({ success: true, message: "Notification logged successfully" });
});
